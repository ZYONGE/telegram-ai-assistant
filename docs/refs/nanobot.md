...
- **기억 계층 분리.** 우리 프로젝트에서는 이렇게 나눈다.
  - `private/profile.md`: 사용자가 직접 관리. nanobot의 `USER.md` 역할
...
| nanobot | 우리 프로젝트 | 이유 |
|---|---|---|
| 알림 여부를 LLM이 판단 | 알림 게이트는 **코드 규칙**으로 `send_now`/`batch`/`hold`/`drop`을 결정: 조용한 시간, `urgent`, 일일 상한, `ref_id` 중복. 모델은 보낼 문장만 다듬음(가벼운 모델) | 조용한 시간·상한은 코드에서 강제한다는 확정 사항. 결정을 재현하고 테스트할 수 있어야 함 |
