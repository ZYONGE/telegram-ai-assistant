...
| 홈 디렉토리 설정 파일에 평문 비밀번호 저장 | `.env`에만 저장. 수집기 안에서만 읽고 로그·예외 메시지·Event에 넣지 않음 | 절대 규칙 6·7 |
| 실행할 때마다 새로 로그인 | Playwright 세션 상태를 `data/`에 저장해 재사용하고, 만료됐을 때만 재로그인 | 로그인 횟수를 줄여 계정 잠금·추가 인증 위험을 낮춘다 |
